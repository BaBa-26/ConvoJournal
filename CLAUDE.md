...
- **Next.js 15** App Router · TypeScript strict · Tailwind CSS (upgraded from 14.2.35 on 2026-07-27 — 14 was EOL; route handler `params` are now `Promise`s, see Gotchas)
...
- Dev credentials provider requires **three** conditions (hardened 2026-07-27): `NODE_ENV === "development"` **and** `ENABLE_DEV_LOGIN === "true"` **and** a `NEXTAUTH_URL` that doesn't look like an https production domain. It's a passwordless "log in as any email" backdoor, so `NODE_ENV` alone was too thin a guard — one mis-set env var on a hosted deploy would have exposed it. **Set `ENABLE_DEV_LOGIN=true` in `.env.local` or dev sign-in silently won't appear.**
...
The CSP lives **only** in `middleware.ts` — `next.config.js` deliberately sets no CSP, because a second static header would be *intersected* with the nonce policy by the browser and fight it. Each request gets a fresh base64 nonce, set on both the forwarded **request** headers (Next reads `Content-Security-Policy` off the request to learn which nonce to stamp on its own bootstrap/flight scripts) and the response. `script-src` is `'self' 'nonce-…' 'strict-dynamic'` — no `'unsafe-inline'`; `'unsafe-eval'` is dev-only for React Refresh.

⚠️ **`export const dynamic = "force-dynamic"` in `app/layout.tsx` is load-bearing.** With `'strict-dynamic'`, a prerendered page would ship a build-time nonce that no longer matches the response header, so every script on it would be blocked and the page would white-screen. Removing that export without also dropping `'strict-dynamic'` breaks the whole app. Verified in prod: every HTML route builds as `ƒ (Dynamic)`; the only `○ (Static)` routes are `/icon`, `/manifest.json`, `/opengraph-image`, which contain no scripts.

`style-src` keeps `'unsafe-inline'` (next/font + the landing page's inline `<style>`); injected CSS isn't script execution. **If you add a third-party script, it must be injected by already-trusted JS** (that's what `'strict-dynamic'` permits) — a hand-written `<script src>` in the HTML will be blocked. A `https://vercel.live` frame violation in the console on *preview* deployments is expected and harmless (Vercel's toolbar; it doesn't exist in prod).
...
| `middleware.ts` | Rate limiting + **the nonce CSP** (single source of truth) + HSTS/security headers |
...
- ~~Windows `next build` fails on `/icon`~~ — **FIXED by the Next 15 upgrade (2026-07-27).** `npm run build` now completes locally on Windows, `/icon` included. Local builds are a real verification option again; `npx tsc --noEmit` alone is no longer the only choice.
- **Route handler `params` are async (Next 15).** Dynamic routes take `{ params }: { params: Promise<{ id: string }> }` and must `await params`. All three `[id]` routes (`tasks`, `goals`, `reminders`) follow this; the auth/IDOR check still runs *before* the id is read. Reading `params.id` synchronously is a build error.
- **`sharp` and `postcss` show in `npm audit` via `next` — deliberately not overridden.** Both are nested copies inside `next`, not direct deps. `sharp` (libvips CVEs) is unreachable: the app uses no `next/image` and defines no `remotePatterns`, so the image optimizer never runs. Next's pinned `postcss@8.4.31` is build-time only over our own CSS (our top-level `postcss` is current and clean). Forcing versions inside `next` risks the build for no reachable gain. **If you ever add `next/image`, re-evaluate `sharp`.**
