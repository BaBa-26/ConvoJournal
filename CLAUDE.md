...
**Next session's work:** (1) **HIGH-1 — upgrade Next.js off 14.2.35** (14 `npm audit` advisories, breaking major upgrade, needs a regression pass — the top open security item); (2) **Phase 3 per-item notifications**; (3) two remaining manual-testing bugs — local-mode (device-only) journal saves don't populate Tasks/Goals/Calendar, and a mobile crisis-support-card overlap (do this one first, it's safety-critical). The cross-account local-data leak is now **fixed** (per-account vault key). See `TLDR.md` for full detail.
