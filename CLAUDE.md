...
`JournalScreen` (5-phase state machine) → `POST /api/transcribe` (Groq Whisper, no auth) → `POST /api/analyze` (local parser, no auth) → `POST /api/journal` (auth required, upserts by `userId + date`). Tasks and reminders are created in the same `POST /api/journal` call.
...
| `app/api/analyze/route.ts` | Calls `lib/parser.ts` — swap Gemini here |
...
- `lib/openai.ts` is dead code (pre-Groq) — nothing imports it but it throws if `OPENAI_API_KEY` is missing. Delete it.
