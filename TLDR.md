# Murmur (ConvoJournal) — Handoff Doc

## What It Is

A mobile-first voice journaling PWA. Speak a daily brain-dump → it transcribes, parses Yesterday / Today / Tomorrow sections, extracts tasks + reminders, and saves everything to SQLite. Unauthenticated users can try voice/transcription freely; saving requires a sign-in.
...
app/
  api/
    ...
    transcribe/route.ts          → Whisper transcription (public)
  ...
  layout.tsx                     → Root layout, PWA meta, fonts, AuthProvider
...
components/
  JournalScreen.tsx              → 5-phase entry flow
  ...
  BottomNav.tsx                  → Fixed nav (Journal + Schedule)
...
lib/
  ...
  openai.ts                      → OpenAI client (Whisper only)
...
|-------|--------|------|---------|
| `/api/transcribe` | POST | No | Audio blob → Whisper → `{ text }` |
...
4. **Analyzing** — character-by-character transcript reveal
...
```bash
cp .env.example .env       # fill in OPENAI_API_KEY + NEXTAUTH_SECRET
npm install
...
- Reminders screen not linked in BottomNav — go to `/reminders` directly
